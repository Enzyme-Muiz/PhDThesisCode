function [X, Y_vals, Y_errs] = plot_dremi_curves(x, Y, varargin)
...
matColors = colormap('jet');%{'r', 'g', 'b', 'k'}; %distinguishable_colors(size(Y, 2));
...
function weights = compute_weights(points, loc, type, factor)
    ...
    range = max(points) - min(points); %quantile(points, 0.98) - quantile(points, 0.02);
